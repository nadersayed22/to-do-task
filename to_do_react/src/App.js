...
class App extends React.Component {
  ...
  handleSubmit(e){
    ...
    if(this.state.editing == true){
      url = `http://127.0.0.1:8000/api/todos${ this.state.activeItem.id}/`
      this.setState({
        editing:false
      })
    }
  ...
  strikeUnstrike(task){
    ...
    var csrftoken = this.getCookie('csrftoken')
    var url = `http://127.0.0.1:8000/api/todos/${task.id}/`
      ...
      fetch(url, {
        ...
        body:JSON.stringify({'completed': task.completed, 'title':task.body})
      }).then(() => {
        this.fetchTasks()
      })
